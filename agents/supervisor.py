...
class BaseState(TypedDict):
    ...
    categories: Annotated[List[BaseMessage], add_messages]
...
def categories(state):
    ...
    return {
        "categories": HumanMessage(ans['category'])
    }
...
def search_tool_agent(state):
    ...
    try:
        ...
        return {
            "message": AIMessage(ans["output"] if isinstance(ans, dict) else str(ans))
        }
    except Exception as e:
        logging.error(f"Error in search_tool_agent: {e}")
        return {
            "message": AIMessage(f"Error occurred during search: {str(e)}")
        }
